...
fine to coarse), showing the median grain size $$d_{50}$$, the initial effective
...
friction angle; $$\sigma'_{r0}$$ = initial effective confining stress; $$D_{r0}$$ = initial
...
2. **SOURCES** — every reference used, with its role (data source, site
   ...
   and DOI or URL.
3. **INDEX_PROPERTIES** — $$d_{50}$$, $$C_u$$, $$G_s$$, $$e_{max}$$, $$e_{min}$$, $$\varphi_c$$, with units.
...
- $$e_0$$ and $$D_{r0}$$ describe the state after consolidation, at the start of
  shearing.
...
state of every test ($$e_0$$, $$D_{r0}$$, $$\sigma'_{r0}$$) is catalogued in the `TEST_PROGRAMME`
...
densities are mutually inconsistent with the reported $$e_{max}$$ and $$e_{min}$$; these
are flagged in the notes where known, and users who need strict consistency
should recompute $$D_r$$ from $$e_0$$, $$e_{max}$$, and $$e_{min}$$ — a few reported relative
